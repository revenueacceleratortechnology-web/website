...
import { pages, serviceLinks, categoryLinks, dtcLinks } from '@/lib/pages';
...
export function ContentPage({ path }: { path: string }) {
 ...
 return <><Header/><main id="main">
  ...
  {page.sections.filter(s=>!s.title.includes('Client')&&!s.title.includes('Delivered')).map((section,i)=><section id={`section-${i}`} key={i} className={`band border-t border-hairline ${i%2?'bg-mineral':'bg-paper'}`}><div className="shell">
   <div className="mb-9 max-w-3xl"><p className="eyebrow text-signal">{String(i+1).padStart(2,'0')} / {path==='about'?'Our story':'A clear path forward'}</p><h2 className="display mt-4 text-[length:var(--text-h2)]">{section.title}</h2>{section.body.map((p,j)=><p key={j} className="mt-5 text-base leading-relaxed text-ash">{p}</p>)}</div>
   {section.cards.length>0&&<div className={`grid gap-5 ${section.cards.length===1?'md:grid-cols-1':'md:grid-cols-2'} ${section.cards.length>2?'lg:grid-cols-3':''}`}>{section.cards.map((c,j)=><article key={j} className="rounded-xl border border-hairline bg-white p-7"><div className="mb-5 flex h-10 w-10 items-center justify-center rounded-lg bg-signal-soft font-bold text-signal">↗</div><h3 className="text-xl font-bold leading-snug">{c.title}</h3>{c.body.map((p,k)=><p className="mt-3 text-sm leading-7 text-ash" key={k}>{p}</p>)}</article>)}</div>}
  </div></section>)}
